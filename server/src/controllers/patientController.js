...
exports.deletePatient = async (req, res) => {
    try {
        ...
        // SECURITY: Only assigned ASHA or ADMIN
        if (role !== 'ADMIN' && patientDoc.data().assigned_to !== uid) {
            return res.status(403).json({ message: 'Unauthorized' });
        }

        await db.collection('patients').doc(patientId).delete();

        // Cleanup: Delete screenings
        const screeningsSnap = await db.collection('screenings').where('patientId', '==', patientId).get();
        const batch = db.batch();
        screeningsSnap.forEach(doc => batch.delete(doc.ref));
        await batch.commit();

        res.status(200).json({ message: 'Patient and screenings cleared' });
    } catch (error) {
